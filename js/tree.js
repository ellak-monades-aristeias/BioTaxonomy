            ...
            for (var j in wikiArray) {
                ...
                if ($.inArray(src, dbArray) > -1) {
                    ...
                    animals_html = animals_html +
                        ...
                        "'width='50px' > <div class='caption'><p caption='" +
                        src + "'>" + src + "</p></div></div></li>";
...
function getName(obj) {
    return obj.closest('.thumbnail').find('.caption>p[caption]').attr(
        'caption');
}
...
function ChangeDiv(width) {
        if (width <= 480) {
            //put img in button
            $(".col-sm-1>.thumbnail>img").wrap(
                '<button class="btn btn-info details" data-target="#myModal" data-toggle="modal" type="button"> </button>'
            );
